...
def busca_lista_dupla(matriz, num):
    ...
    idxLista = pesquisaBinaria(matriz, lambda x: 0 if x[0] <= num and num <= x[-1] else (-1 if x[0] > num else 1))
    return pesquisaBinaria(matriz[idxLista], lambda x: 0 if x == num else (-1 if x > num else 1)) != -1
